...
st.session_state.API_KEY_WRITE=os.environ.get('API_KEY_WRITE') # github pats
...
with st.sidebar:
    ...
    view_customer=st.selectbox("Select Customer",
                ...
                index=idx)
    st.divider()
    data_map = getCustomerDataMap(user,view_customer) # calls customers/customer/data_map.json
    ...
    if toggle_state:
        ...
        try:
            ...
            if view_customer and not customer: # view customer selected, edit customer unselected
                data_map = getCustomerDataMap(user,view_customer) # want to display different data_maps
